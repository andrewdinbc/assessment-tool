...
export default function AdminPage() {
  ...
  return (
    <div style={{ padding: 32, fontFamily: 'Georgia, serif', maxWidth: 800 }}>
      <h1 style={{ color: C.navy, fontSize: 24, marginBottom: 4 }}>Parent Contact Admin</h1>
      ...
      <form onSubmit={addContact} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20, marginBottom: 24, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <input
          value={qrId}
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: 20, color: C.muted, fontStyle: 'italic' }}>No contacts registered yet.</div>
        ) : (
          contacts.map((c) => (
      ...
      <div style={{ marginTop: 24, fontSize: 12, color: C.muted }}>
        Each family&apos;s portal link: <code>/portal/[QR ID]</code> on parent-portal — give this to families out of band
        (printed card, welcome email).
      </div>
    </div>
  )
}
